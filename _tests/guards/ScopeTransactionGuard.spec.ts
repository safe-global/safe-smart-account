...
describe("ScopeTransactionGuard", async () => {
  ...
  describe("checkTransaction", async () => {
    it("should revert delegate call", async () => {
      ...
      ).to.be.revertedWith("DelegateCall not allowed to this address");
    });
    ...
    it("can allow target with safe", async () => {
      const { safe, guard } = await setupTests();
      expect(await guard.isAllowedTarget(AddressZero)).to.be.eq(false);

      await guard.allowTarget(AddressZero);

      expect(await guard.isAllowedTarget(AddressZero)).to.be.eq(true);
    });

    it("can disallow target with safe", async () => {
      const { safe, guard } = await setupTests();
