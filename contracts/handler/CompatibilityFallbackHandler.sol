...
contract CompatibilityFallbackHandler is TokenCallbackHandler, ISignatureValidator, HandlerContext {
    ...
    function getMessageHash(bytes memory message) public view returns (bytes32) {
        return getMessageHashForSafe(ISafeExtended(payable(msg.sender)), message);
    }
     ...
     */
    function encodeMessageDataForSafe(ISafeExtended safe, bytes memory message) public view returns (bytes memory) {
     ...
     * @return Message hash.
     */
    function getMessageHashForSafe(ISafeExtended safe, bytes memory message) public view returns (bytes32) {
    ...
    function isValidSignature(bytes32 _dataHash, bytes calldata _signature) public view override returns (bytes4) {
        ...
        ISafeExtended safe = ISafeExtended(payable(msg.sender));
    ...
    function checkNSignatures(
        ...
        ISafeExtended(payable(_manager())).checkNSignatures(_msgSender(), dataHash, signatures, requiredSignatures);
    }
}
