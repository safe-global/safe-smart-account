...
describe("GnosisSafe", async () => {
    ...
    describe("requiredTxGas", async () => {
        ...
        it('should not return estimate when not called from Safe', async () => {
        ...
        it('should return estimate in revert', async () => {
            ...
            await readOnlySafe.callStatic.requiredTxGas(
                ...
                expect(false, "Should never be successful")
            }, (error) => { 
                const message: string = error.message
                // We use some reasonable maximum just to check if something meaningfull is returned
                expect(BigNumber.from(Buffer.from(message).slice(-32)).toNumber()).to.be.lt(10000)
            })
        })
    })
})