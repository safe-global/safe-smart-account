...
describe("GnosisSafe", async () => {
    ...
    describe("requiredTxGas", async () => {
        it("should revert without reason if tx fails", async () => {
            ...
            await readOnlySafe.callStatic.requiredTxGas(safe.address, 0, data, 0, { from: safe.address }).then(
                () => {
                    throw Error("Should never be successful")
                },
                (error) => {
                    const message: string = error.message
                    expect(message.includes("call revert exception")).to.be.true
                },
            )
        })
        ...
        it("should always revert", async () => {
            ...
            const readOnlySafe = safe.connect(hre.ethers.provider)
            await readOnlySafe.callStatic.requiredTxGas(safe.address, 0, "0x", 0, { from: safe.address }).then(
                () => {
                    throw Error("Should never be successful")
                },
                (error) => {
