...
import { deployContractFromSource, getMock, getMultiSendCallOnly, getSafe } from "../utils/setup";
...
describe("MultiSendCallOnly", async () => {
    ...
    describe("multiSend", async () => {
        ...
        it("Can execute single ether transfer", async () => {
            ...
            await expect(await hre.ethers.provider.getBalance(safeAddress)).to.eq(hre.ethers.parseEther("0"));
            await expect(await hre.ethers.provider.getBalance(user2.address)).to.eq(userBalance + hre.ethers.parseEther("1"));
        });
