...
describe("OutSafe - e2e", async () => {
    ...
    const setupTests = deployments.createFixture(async ({ deployments }) => {
        ...
        await executeContractCallWithSigners(outSafe, outSafe, "setAsset", [erc20.address, 1], [admin1, admin2, admin3]);
    ...
    it("should not be able to have verifier address added directly", async () => {
        const { outSafe } = await setupTests();
        await expect(outSafe.setVerifier(hotWallet1.address, [AddressZero], [ethers.utils.parseEther("5.0")])).to.be.revertedWith("GS031");
    });
    ...
    it("should allow ETH withdrawal from a hot wallet within limit", async () => {
        ...
        const response = await outSafe
            .connect(user2)
            .withdraw(withdrawal1.asset, withdrawal1.amount, withdrawal1.nonce, withdrawal1.expiry, sig1);
        ...
        await outSafe
            .connect(user2)
            .withdrawTo(user3.address, withdrawal2.asset, withdrawal2.amount, withdrawal2.nonce, withdrawal2.expiry, sig2);
    ...
    it("should not allow ETH withdrawal from a hot wallet over limit", async () => {
        ...
        await expect(
            outSafe
                .connect(user3)
                .withdrawTo(user2.address, withdrawal2.asset, withdrawal2.amount, withdrawal2.nonce, withdrawal2.expiry, sig2),
    ...
    it("should allow/disallow ERC20 withdrawal like ETH withdrawal", async () => {
        ...
        await erc20.transfer(user1.address, 2000);
        await erc20.connect(user1).transfer(outSafe.address, 2000);
        ...
        await outSafe.connect(user2).withdraw(withdrawal1.asset, withdrawal1.amount, withdrawal1.nonce, withdrawal1.expiry, sig1);
        ...
        expect(await erc20.balanceOf(user3.address)).to.be.equal(200);

        const withdrawal3: Withdrawal = {
            to: user2.address,
            asset: erc20.address,
            amount: 200,
            nonce: 4,
        ...
        const { data: sig3 } = await signWithdrawalData(hotWallet1, outSafe, withdrawal3);

        await expect(
            outSafe
                .connect(user3)
                .withdrawTo(user2.address, withdrawal3.asset, withdrawal3.amount, withdrawal3.nonce, withdrawal3.expiry, sig3),
        ).to.be.revertedWith("OS04");
    });

    it("should not allow withdrawal from a unrecognized wallet", async () => {
        const { outSafe } = await setupTests();
        await outSafe.connect(user1).deposit({
            value: ethers.utils.parseEther("5.0"),
        });
        ...
        await expect(
            outSafe
                .connect(user2)
                .withdrawTo(user2.address, withdrawal1.asset, withdrawal1.amount, withdrawal1.nonce, withdrawal1.expiry, sig1),
    ...
    it("should not allow withdrawal from a invalid signature", async () => {
        ...
        await expect(
            outSafe
                .connect(user2)
                .withdrawTo(user3.address, withdrawal1.asset, ethers.utils.parseEther("3.0"), withdrawal1.nonce, withdrawal1.expiry, sig1),
        ).to.be.revertedWith("OS03");
    });
    ...
    it("should not allow withdrawal with the same nonce", async () => {
        ...
        await expect(
            outSafe.connect(user2).withdraw(withdrawal1.asset, withdrawal1.amount, withdrawal1.nonce, withdrawal1.expiry, sig1),
